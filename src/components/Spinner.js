    ...
    if (names.length > 0) {
      ...
      const rotateWheel = () => {
        ...
        if (spinTime >= spinTimeTotal) {
          const degrees = (angle * 180) / Math.PI + 90; // +90 to align with the top position of the arrow
  ...
  return (
    <div className="text-center m-11">
      ...
      <div className="flex flex-row justify-center items-center">
        <div className="flex flex-1 justify-normal items-center">
          <div className="wheel-container">
            ...
            <button
              className="rounded-md text-white min-w-[150px] bg-blue-800 m-5 p-3"
              onClick={spinWheel}
              disabled={isSpinning}
            >
              Spin it!
        ...
        <div className="flex-1 justify-center items-center border border-black rounded-lg m-5 p-3">
          <div className="border-b-2">
            <input
              type="text"
              className="border border-slate-800 p-2 m-2"
              value={newName}
              onChange={(e) => setNewName(e.target.value)}
              placeholder="Enter a name"
            />
            <button className="rounded-lg bg-blue-500" onClick={addName}>
              Add Name
            </button>
          </div>
          <div className=" text-left top-[1em] text-xl">
            <ul>
      ...
      <div className="flex flex-1 justify-center items-center">
        {selectedName && (
          <div className="selected-name align-middle text-center w-[400px] h-[70px] rounded-lg text-2xl text-white bg-black m-2 p-3">
